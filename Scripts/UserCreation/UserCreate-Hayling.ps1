...
$LearnersHomeDir = "\\THC-FS-02\Home$\2022_Intake\"
...
foreach ($usr in $UsersFromCSV) {
    ...
    # }
    # Students
    if ($usr.OU -match "_20\d\d") {
        $LeaversGroup = $usr.OU
        $usrOU = $usr.OU
        $usr.OU = "OU=_2022,OU=Intake,OU=STUDENTS,OU=USERS,OU=THC,DC=THC,DC=local"
    }
    ...
    # Students
    if ($usr.OU -match 'OU=_20\d\d') {
        Add-ADPrincipalGroupMembership -Identity $usr.UserName -MemberOf "THC_WiFi"
    }      
    ...
    #Students 
    if ($usr.OU -match 'OU=_20\d\d') {
        $homepath = -join ($LearnersHomeDir, $usr.UserName)
        Set-ADUser -Identity $usr.UserName -HomeDirectory "$homepath" -HomeDrive P 
        New-Item -Path $homepath -ItemType Directory
        # Add permissions
        $acl = Get-Acl $homepath
        $adusr = Get-ADUser -Identity $usr.UserName
